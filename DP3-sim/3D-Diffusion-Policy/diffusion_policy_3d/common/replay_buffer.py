...
class ReplayBuffer:
    ...
    def __init__(self, 
        ...
        for key, value in root['data'].items() if hasattr(root['data'], 'items') else root['data'].members():
    ...
    def copy_from_store(cls, src_store, store=None, keys=None, 
        ...
        if store is None:
            # numpy backend
            meta = dict()
            for key, value in src_root['meta'].items() if hasattr(src_root['meta'], 'items') else src_root['meta'].members():
    ...
    def copy_from_path(cls, zarr_path, backend=None, store=None, keys=None, 
        ...
        if backend == 'numpy':
            ...
            store = None
        # Zarr v3 uses explicit keyword mode
    ...
    def save_to_store(self, store, 
        ...
        if self.backend == 'zarr':
            ...
            for key, value in self.root['meta'].items() if hasattr(self.root['meta'], 'items') else self.root['meta'].members():
                _ = meta_group.array(
                    name=key,
                    data=value, 
                    shape=value.shape, 
        ...
        # save data, chunk
        data_group = root.create_group('data', overwrite=True)
        for key, value in self.root['data'].items() if hasattr(self.root['data'], 'items') else self.root['data'].members():
